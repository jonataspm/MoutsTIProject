...
public class CartsController : BaseController
{
    ...
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> GetCarts(CancellationToken cancellationToken, [FromQuery] string? _order, [FromQuery] int? _page = 1, [FromQuery] int? _size = 10)
    {
        var request = new ListCartsRequest { Page = _page, Size = _size, Order = _order };
    ...
    public async Task<IActionResult> UpdateCart([FromRoute] Guid id, [FromBody] UpdateCartRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!validationResult.IsValid)
            return BadRequest(validationResult.Errors.Select(e => e.ErrorMessage).FirstOrDefault() ?? "Validation failed");
